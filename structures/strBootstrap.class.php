...
class strBootstrap{
 ...
 public function render($echo=true){
  ...
  $return.="  <title>".($this->title?$this->title." - ":null)."Ambrogio</title>\n";
  $return.=" </head>\n";
  $return.=" <body>\n";
  ...
  // cycle all alerts
  if(is_array($_SESSION['ambrogio']['alerts']) && count($_SESSION['ambrogio']['alerts'])){
   $return.="  <!-- alerts -->\n";
   $return.="  <script type=\"text/javascript\">\n";
   foreach($_SESSION['ambrogio']['alerts'] as $index=>$alert){
    // swicth class
    switch($alert->class){
    ...
    // remove from session
    unset($_SESSION['ambrogio']['alerts'][$index]);
   }
   $return.="  </script>\n";
   $return.="  <!-- /alerts -->\n";
  }
